...
In elite level football, this difficulty has been addressed through the introduction of a semi-automated offside system which consists of dedicated multi-camera infrastructure and proprietary tracking software. However, this technology is limited to the top tier of the sport given its capital cost and operational complexity. As a result, any league or game that takes place outside these elite level competitions does not have access to this infrastructure, and offside decisions in those contexts are based solely on the linesman's judgement.
...
The first assumption is that the input footage uses a standard wide-angle main broadcast view, which is typically employed for live coverage. The system cannot operate on close-up shots since it requires a minimum number of visible pitch landmarks; a standard wide-angle view provides sufficient coverage to carry out the full pipeline.

The second assumption is that the keyframe corresponding to the moment of the pass is selected manually rather than detected automatically. Identifying the precise frame at which the ball leaves the passing player's foot is crucial, since even one frame of error can change the attacker's classification from offside to onside given the speed of play. Removing this manual step is identified as future work rather than a contribution of the present work.
...
The remainder of this report is organised as follows. Chapter~\ref{chp:background} covers the offside law, object detection, projective geometry, and a review of related work. Chapter~\ref{chp:design} details the pipeline design and motivates component choices. Chapter~\ref{chp:implementation} documents each stage and the supporting annotation interfaces. Chapter~\ref{chp:evaluation} evaluates accuracy against ground-truth annotations and analyses failure modes. Chapters~\ref{chp:future_work} and~\ref{chp:conclusions} outline future directions and conclusions.
