...
def get_player_colour(img, box, grass_colour, mask=None):
    ...
    # IMPROVEMENT 1: arm-skip horizontal trim. Sleeves and skin at the bbox
    # edges bleed into the median Lab and pull the cluster — trimming the
    # outer 18% on each side keeps mostly torso pixels.
    ...
    # IMPROVEMENT 2: when a segmentation mask is available, sample only pixels
    # inside the mask in the torso ROI. If the mask is empty in this ROI,
    # fall back to the rectangular crop so the function still returns a colour.
    used_mask = False
    if mask is not None:
...
def identify_teams(img, boxes, masks, grass_colour, debug=False):
    ...
    # IMPROVEMENT 6: up-weight chroma (a, b) channels by 1.8x. Team kits typically
    # differ in hue more than luminance, so chroma should drive the K-means split.
    # Reduces L-driven mis-clusters (white vs yellow teams; shadow vs sun within
    # the same team).
...
for test_kf in keyframes:
    ...
    labels, valid_boxes = identify_teams(img, pitch_boxes, pitch_masks, grass_colour)
    if len(labels) > 0:
