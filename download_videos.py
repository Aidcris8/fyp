# Downloads match videos from the SoccerNet dataset for the games selected using
# scan_offsides.py. Each game's two halves are saved as 720p mkv files under
# data/soccernet/. Touches a stamp file so Make knows new data is available.

import os
from SoccerNet.Downloader import SoccerNetDownloader

#SoccerNet password is publicly documented in the dataset paper
PASSWORD = "s0cc3rn3t"

#initialise downloader pointing at the local data directory
downloader = SoccerNetDownloader(LocalDirectory="data/soccernet")
downloader.password = PASSWORD

#list of games you desire to download
games = [
    # games that I already downloaded acrodd five leagues to maximise coverage
    # games must follow league/season/date - teams and result format
...
#iterate through the list and download both halves of each game as 720p mkv files
for game in games:
    print(f"Downloading: {game}")
    downloader.downloadGame(
        game=game,
        files=["1_720p.mkv", "2_720p.mkv"] # 1 being 1st half and 2 being 2nd half
    )
    print(f"Done: {game}\n")
